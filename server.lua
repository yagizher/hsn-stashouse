...
AddEventHandler('hsn-stashouse:addnewowner', function( targetplayer,evid)
  ...
  targetplayer = targetplayer
	...
	if targetplayer ~= nil then
		if houseid ~= nil then
					MySQL.Async.fetchAll('SELECT * FROM hasan_stashouse WHERE identity = @identity', {
						['@identity'] = targetplayer.identifier,
					}, function(result2)
						if result2[1] == nil then
							MySQL.Async.execute('INSERT INTO hasan_stashouse (identity, houseid) VALUES (@identity, @houseid)',{
...
ESX.RegisterServerCallback('hsn-stashouse:getid', function(source, cb, houseid)
  ...
  MySQL.Async.fetchScalar("SELECT houseid FROM hasan_stashouse WHERE identity=@identity ", {['@identity'] = playerIdent, ['@houseid'] = houseid}, function(rentalID)
...
ESX.RegisterServerCallback('hsn-stashouse:getprice', function(source, cb, price)
  ...
  MySQL.Async.fetchScalar("SELECT price FROM hasan_stashouse WHERE identity=@identity ", {['@identity'] = playerIdent, ['@price'] = price}, function(rentalID)
      if rentalID ~= nil then
          cb(rentalID)
      else
          cb(false)
...
ESX.RegisterServerCallback('hsn-stashouse:checkIsOwner', function(source, cb, houseid)
  ...
  MySQL.Async.fetchScalar("SELECT houseid FROM hasan_stashouse WHERE houseid = @houseid AND identity = @identity", {
      ['@houseid'] = houseid,
  ...
  local _source = source
    ...
    if money >= price then
      MySQL.Async.fetchAll('SELECT * FROM hasan_stashouse WHERE identity = @identity', {
        ...
        if result2[1] == nil then
    MySQL.Async.fetchAll('SELECT * FROM hasan_stashouse WHERE houseid = @houseid', {
      ['@houseid'] = houseid,
    }, function(result)
      if result[1] == nil then
      MySQL.Async.execute('INSERT INTO hasan_stashouse (identity, houseid,price) VALUES (@identity, @houseid,@price)',
        {
          ['@identity']   = pl.identifier,
          ['@houseid']    = houseid,
          ['@price'] = price,
...
AddEventHandler('hsn-stashouse:detelehouseownership', function(evinidsi,price)
  local _source = source
    ...
    MySQL.Sync.execute("DELETE FROM hasan_stashouse WHERE identity=@identity  AND  houseid=@houseid", {['@identity'] = pl.identifier, ['@houseid'] = evinidsi, })
    pl.addMoney(amount)
